...
if (isset($sFilter)) {
    ...
    if (strstr($sFilter, ' ')) {
        ...
        $sFilterWhereExt = " AND per_LastName LIKE '%".$aFilter[0]."%' ".
                            " AND per_FirstName LIKE '%".$aFilter[1]."%' ";
    } else {
        $sFilterWhereExt = " AND (per_LastName LIKE '%".$sFilter."%' ".
                            " OR per_FirstName LIKE '%".$sFilter."%') ";
    }
}
...
switch ($sSort) {
    case 'family':
            ...
            break;
    case 'zip':
            $sOrderSQL = ' ORDER BY zip, per_FirstName, per_LastName';
            break;
    case 'entered':
    ...
    case 'edited':
	 ...
	 case 'lastname':
            ...
            break;      
    default:
            $sOrderSQL = ' ORDER BY per_FirstName, per_LastName';
            break;
}
...
// Run query to get first letters of first name.
$sSQL = 'SELECT DISTINCT LEFT(per_FirstName,1) AS letter FROM person_per LEFT JOIN family_fam ON per_fam_ID = family_fam.fam_ID '.
...
<select name="Sort" onchange="this.form.submit()">
    ...
    echo 'selected';
} ?>><?= gettext('Familia') ?></option>
		<option value="lastname" <?php if ($sSort == 'lastname') {
    echo 'selected';
} ?>><?= gettext('Ultimo Nome') ?></option>
		<option value="zip" <?php if ($sSort == 'zip') {
    echo 'selected';
} ?>><?= gettext('CEP') ?></option>
		<option value="entered" <?php if ($sSort == 'entered') {
    echo 'selected';
} ?>><?= gettext('Mais Recentes') ?></option>
		<option value="edited" <?php if ($sSort == 'edited') {
    echo 'selected';
} ?>><?= gettext('Recentemente Editados') ?></option>
...
while ($aRow = mysqli_fetch_array($rsPersons)) {
    ...
    <td><img data-src="<?= SystemURLs::getRootPath(); ?>/api/persons/<?= $per_ID ?>/thumbnail" data-name="<?= $per_LastName.' '.$per_FirstName ?>" class="initials-image direct-chat-img " width="10px" height="10px" /> </td>
	<td>
	    <a href="PersonView.php?PersonID=<?= $per_ID ?>" >
	    <?= FormatFullName($per_Title, $per_FirstName, $per_MiddleName, $per_LastName, $per_Suffix) ?>
	    </a>
    </td>
    <td>
    <?php
